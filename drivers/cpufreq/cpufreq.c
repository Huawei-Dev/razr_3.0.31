...
#define SYSTEM_PWRSAVE_MODE 0
#define SYSTEM_NORMAL_MODE 1
#define SYSTEM_OVERCLOCK_MODE 2
...
{							\
        policy->cpuinfo.min_freq = 150000;      	\
...
static int system_mode_set(const char *arg, const struct kernel_param *kp)
{
	...
	if (ret == 0) {
		printk("system_mode_set system_mode=%u\n",system_mode);
 		...
 		if (system_mode <= 0)
			system_mode = SYSTEM_PWRSAVE_MODE;
		if (system_mode == 1) 
			system_mode = SYSTEM_NORMAL_MODE;
 		if (system_mode >= 2) 
			system_mode = SYSTEM_OVERCLOCK_MODE;
...
static ssize_t store_cpuinfo_max_freq(struct cpufreq_policy *policy, const char *buf, size_t count)
{
   unsigned int ret = -EINVAL;
   ...
   //if (!cpu_online(policy->cpu)) cpu_down(policy->cpu);

  ret = cpufreq_get_policy(&new_policy, policy->cpu);
  new_policy.max = value;
  ret = __cpufreq_set_policy(policy, &new_policy);
  policy->user_policy.max = policy->max = policy->cpuinfo.max_freq = value;
